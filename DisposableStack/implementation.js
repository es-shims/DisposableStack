...
CreateMethodProperty(DisposableStack.prototype, 'dispose', function dispose() {
	...
	return DisposeResources(SLOT.get(disposableStack, '[[DisposableResourceStack]]'), NormalCompletion())['?'](); // step 5
});
...
CreateMethodProperty(DisposableStack.prototype, 'use', function use(value) {
	...
	AddDisposableResource(SLOT.get(disposableStack, '[[DisposableResourceStack]]'), value, '~SYNC-DISPOSE~'); // step 4
...
CreateMethodProperty(DisposableStack.prototype, 'adopt', function adopt(value, onDispose) {
	...
	AddDisposableResource(SLOT.get(disposableStack, '[[DisposableResourceStack]]'), void undefined, '~SYNC-DISPOSE~', F); // step 7
...
CreateMethodProperty(DisposableStack.prototype, 'defer', function defer(onDispose) {
	...
	AddDisposableResource(SLOT.get(disposableStack, '[[DisposableResourceStack]]'), void undefined, '~SYNC-DISPOSE~', onDispose); // step 5
});
...
CreateMethodProperty(DisposableStack.prototype, 'move', function move() {
	...
	var newDisposableStack = new DisposableStack(); // steps 4-5
	SLOT.set(newDisposableStack, '[[DisposableResourceStack]]', SLOT.get(disposableStack, '[[DisposableResourceStack]]')); // step 6
	SLOT.set(disposableStack, '[[DisposableResourceStack]]', []); // step 7
